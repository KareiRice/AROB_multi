    ...
    try:
        for num, line in enumerate(lines):
            if not line[6] == '' and not emotion_dict[line[5]] == 0:
                data.append([np.concatenate((past_info1, past_info2, line[12])), 1 if emotion_dict[line[5]] == 1 else 0, line[6]])
            if not line[7] == '' and not emotion_dict[line[5]] == 0:
                data.append([np.concatenate((past_info1, past_info2, line[13])), 1 if emotion_dict[line[5]] == 1 else 0, line[7]])
...
if __name__ == '__main__':
    epochs = 300
    X_VALID = 10
    SAVE_DIR = './hist_past3/extract/X{}'.format(X_VALID)
    data = load_dataset()
    ...
    for time in range(X_VALID):
        model = Model.StackedGRU(2, EMBED_SIZE, EMBED_SIZE*2, HIDDEN_SIZE, CATEGORIES)
        log_train.append([])
        log_test.append([])
