...
def load_dataset():
    ...
    data = []
    kara = lines[0][12]
    past_info1 = np.concatenate((kara, kara))
    past_info2 = np.concatenate((kara, kara))
    past_info3 = np.concatenate((kara, kara))
    try:
        for num, line in enumerate(lines):
            if not line[6] == '' and not emotion_dict[line[5]] == 0:
                data.append(
                    [np.concatenate((past_info1, past_info2, past_info3, line[12])), 1 if emotion_dict[line[5]] == 1 else 0, line[6]])
            if not line[7] == '' and not emotion_dict[line[5]] == 0:
                data.append(
                    [np.concatenate((past_info1, past_info2, past_info3, line[13])), 1 if emotion_dict[line[5]] == 1 else 0, line[7]])
            past_info3 = past_info2[:]
            past_info2 = past_info1[:]
            past_info1 = np.concatenate((line[12], line[13]))
        print(num)
    except ImportError:
        pass
...
if __name__ == '__main__':
    ...
    SAVE_DIR = './hist_past4/extract/X{}'.format(X_VALID)
    ...
    for time in range(X_VALID):
        model = Model.StackedGRU(3, EMBED_SIZE, EMBED_SIZE*2, HIDDEN_SIZE, CATEGORIES)
        log_train.append([])
        log_test.append([])
        log_test_categories.append([])
        data = data[valid_length * time:] + data[:valid_length * time]
        X, y, sentences = np.array(np.array(data)[:, 0].tolist()), np.array(np.array(data)[:, 1].tolist()), np.array(
            np.array(data)[:, 2].tolist())
